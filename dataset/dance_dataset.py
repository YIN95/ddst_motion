...
class AISTPPDataset(Dataset):
    def __init__(
        ...
        self.raw_fps = 60
        self.data_fps = 30
        self.style_list = ['gBR', 'gHO', 'gJB', 'gJS', 'gKR', 'gLH', 'gLO', 'gMH', 'gPO', 'gWA']
        ...
        pickle_name = f"processed_train_data_{data_sub}.pkl" if train else f"processed_test_data_{data_sub}.pkl"
        ...
        if not train:
            pickle.dump(
                normalizer, open(os.path.join(backup_path, f"normalizer_{data_sub}.pkl"), "wb")
            )
        # load raw data
    ...
    def __getitem__(self, idx):
        ...
        # print(feature.shape)
        return self.data["pose"][idx], feature, filename_, self.data["wavs"][idx], self.data["labels"][idx]
    ...
    def load_aistpp(self):
        ...
        if self.data_sub is not "all":
            motions = [item for item in motions if self.data_sub in item]
            features = [item for item in features if self.data_sub in item]
            wavs = [item for item in wavs if self.data_sub in item]
        ...
        for motion, feature, wav in zip(motions, features, wavs):
            ...
            style = m_name.split('_')[0]
            style_index = self.style_list.index(style)
        ...
        data = {"pos": all_pos, "q": all_q, "filenames": all_names, "wavs": all_wavs, "labels": all_index}
        return data
