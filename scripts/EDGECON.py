...
from dataset.dance_dataset_con import AISTPPDataset
...
class EDGE:
    ...
    def train_loop(self, opt):
        # load datasets
        # if opt.data_sub == "all":
        #     data_cache_train = "train_tensor_dataset"
        #     data_cache_test = "test_tensor_dataset"
        # else:
        data_cache_train = f"train_tensor_dataset_{opt.data_sub}"
        data_cache_test = f"test_tensor_dataset_{opt.data_sub}"
        ...
        for epoch in range(1, opt.epochs + 1):
            ...
            # train
            self.train()
            for step, (x, cond, filename, wavnames, labels) in enumerate(
            ...
            if (epoch % opt.save_interval) == 0:
                ...
                if self.accelerator.is_main_process:
                    ...
                    (x, cond, filename, wavnames, labels) = next(iter(test_data_loader))
    ...
    def render_sample(
        ...
        latent, cond, wavname = data_tuple
    ...
    def render_sample_source(
        ...
        sample, cond, wavname = data_tuple
        assert len(cond.shape) == 3
        if render_count < 0:
            render_count = len(cond)
        cond = cond.to(self.accelerator.device)
